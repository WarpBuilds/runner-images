...
- Install Powershell, Packer and Azure CLI on your local machine and login to Azure CLI.
- Drop into Powershell
  ```sh
  pwsh
  ```
- Go to cloned directory of this repo and run:
  ```ps
  Set-Location . 
  ```
- Run this command to build the image on Azure. **Make sure to change the ResourceGroupName and SubscriptionId**. The resource group should be new and unique. 
  ```ps
  GenerateResourcesAndImage -SubscriptionId "subscription-id" -ResourceGroupName "unique-resource-group-name" -ImageType Ubuntu2204 -AzureLocation "East US"
  ```
- A new image named `Runner-Image-Ubuntu2204` would be created in the new resource group.
